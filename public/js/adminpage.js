...
$(document).ready(function () {
  ...
  $("#back").on("click", function () {
    updateListing(getParentDirectory(currentURL));
  });
 ...
 $("#select").on("mouseover", "#select>li>.listItem", function () {
    $(this).popover("show")  });

  $("#select").on("mouseout", "#select>li>.listItem", function () {
    $(this).popover("hide")  });
...
function getLastDirectory(url){
  var a = url.split("/");
  a.splice(-1,1);
  var b = [a[0], a[1], a[2], "...", a[a.length-1]]
  var c = b.join("/");
  return c;
}
...
function rebuildSelectList(){
  ...
  getSelected(function (data){
    if (!data || data == "") {
      console.log("no data");}
      for (var i in data) {
        var url = getLastDirectory(data[i]);
        $("#select").append("<li><button type='button' class='listbtn btn btn-xs btn-default'>-</button><a class='listItem' data-toggle='popover' data-trigger='manual' data-placement='top' data-content='" + data[i] + "' data-dir='" + data[i] + "' href='#'> "
          + url + "</a></li>");}
      })
}
